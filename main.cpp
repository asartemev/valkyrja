...
int main(int argc, char** argv) {
    ...
    vector<set<hwcpipe_counter>> bundle_sets = {
        ...
        {MaliAnyUtil, MaliFragActiveCy, MaliCompQueueActiveCy},              // Bundle 1: High level activity
        ...
        {MaliExtBusRdBy, MaliExtBusWrBy}                                     // Bundle 4: Ext Bus Memory
    };
    ...
    while (keep_running) {
        ...
        double alu_load_val = 0.0;
        if (gpu_util > 0.001) {
            alu_load_val = (alu_util / gpu_util) * 100.0;
            if (alu_load_val > 100.0) alu_load_val = 100.0;
            stringstream ss; ss << fixed << setprecision(2) << alu_load_val << " %";
            alu_load_str = ss.str();
        }
        // L2 CACHE HIT RATE
        double l2_hit_rate_val = 100.0 - l2_miss;
        if (l2_hit_rate_val < 0.0) l2_hit_rate_val = 0.0;
        stringstream ss_l2; ss_l2 << fixed << setprecision(2) << l2_hit_rate_val << " %";

        // MEMORY LOAD BOUND (Back-End Bound Equivalent)
        double memory_load_bound_val = ls_stall * (l2_miss / 100.0);
        if (memory_load_bound_val > 100.0) memory_load_bound_val = 100.0;
        stringstream ss_mlb; ss_mlb << fixed << setprecision(2) << memory_load_bound_val << " %";

        // --- ASSIGN ANSI DYNAMIC COLOR LOGIC TO THE DERIVED OUTPUT ENGINE ---
        string color_alu = "\033[33m"; // Yellow Default
        if (alu_load_val > 40.0) color_alu = "\033[32m"; // Green: Dense math processing
        else if (alu_load_val < 20.0 && gpu_util > 50.0) color_alu = "\033[31m"; // Red: GPU busy but running tiny math

        string color_l2 = "\033[32m"; // Green Default
        if (l2_hit_rate_val < 75.0) color_l2 = "\033[31m"; // Red: Terribly un-aligned cache layout
        else if (l2_hit_rate_val < 85.0) color_l2 = "\033[33m"; // Yellow: Cache misses rising
        ...
        if (memory_load_bound_val > 15.0) color_mlb = "\033[31m"; // Red: Major system RAM bottlenecking
        else if (memory_load_bound_val > 5.0) color_mlb = "\033[33m"; // Yellow: Medium load constraints
        ...
        cout << setw(25) << left << "ALU_LOAD_DERIVED" << " : " << colorize(alu_load_str, color_alu) << endl;
        cout << setw(25) << left << "L2_CACHE_HIT_RATE" << " : " << colorize(ss_l2.str(), color_l2) << endl;
        cout << setw(25) << left << "FRONTEND_STARVE_RATE" << " : " << colorize(stable_starve_rate_str, color_starve) << " (Front-End Bound)" << endl;
